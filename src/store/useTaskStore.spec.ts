...
describe('useTaskStore', () => {
  // ISOLEMENTO: Limpa a store antes de cada teste para evitar vazamento de estado
  beforeEach(() => {
...
it('deve alternar o status de uma tarefa entre pending e done', () => {
    ...
    const taskParaAlterar2 = tasks[2]; // Essa é a tarefa que não vamos alterar, para garantir que o toggle não mexe nela
    
    // Act (Marca a primeira como done)
  ...
  it('deve editar os dados de uma tarefa existente sem perder os dados antigos', async () => {
    ...
    // A nossa pausa para o relógio andar
    await new Promise((resolve) => setTimeout(resolve, 10));

    // Act (Altera apenas o título da primeira tarefa)
    ...
    // A prova de que o ramo falso funcionou: a tarefa 2 não sofreu alterações
    expect(tasksAtualizadas[0].title).toBe('Tarefa Antiga 2'); 
  });

  it('deve excluir a tarefa correta da lista', () => {
    // Arrange (Adiciona duas tarefas)
    ...
    expect(tasksDepois[0].id).not.toBe(idParaExcluir); // A tarefa que sobrou não é a que excluímos
  });
});